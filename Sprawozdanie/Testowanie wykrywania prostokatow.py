...
def main():
    ...
    sciezkaDocelowa = "Testowanie wykrywania prostokatow/"
    ...
    for iKategoriaZdjec in range(len(kategoriaZdjec)):
        ...
        for iTytul in range(len(tytul)):
            ...
            fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(12, 5), subplot_kw={'xticks': [], 'yticks': []})
            fig.subplots_adjust(left=0, right=1, bottom=0, top=1, hspace=0.1, wspace=0.1)
            ax[0].set_title("Ilosc konturów: " + str(len(contours)))
            ax[0].imshow(cv2.cvtColor(imageWszystkieKontury, cv2.COLOR_BGR2RGB), aspect='equal', interpolation='bilinear')
            ax[1].set_title("Ilosc prostokątów: " + str(len(konturyProstokatow)))
            ax[1].imshow(cv2.cvtColor(imageKonturyProstokatow, cv2.COLOR_BGR2RGB), aspect='equal', interpolation='bilinear')
